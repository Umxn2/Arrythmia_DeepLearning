...
def main():
    if torch.backends.mps.is_available():
        ...
        train_directory = '/Users/umang/umang/3-1/Image_Processing/project/augmented'
        valid_directory = '/Users/umang/umang/3-1/Image_Processing/project/augmented_test'
        ...
        dataloaders = {
            'train':data.DataLoader(dataset['train'], batch_size=bs, 
                                    # shuffle=True,
                                    # generator=torch.Generator(device="mps"),
                                    #sampler="RandomSampler",
                                   
                                    num_workers=num_cpu, pin_memory=True, drop_last=True),
            'valid':data.DataLoader(dataset['valid'], batch_size=bs,
                                    #  shuffle=True,
                                    # generator=torch.Generator(device="mps"),
                                     # sampler="RandomSampler",
                                    num_workers=num_cpu, pin_memory=True, drop_last=True)
    ...
    #    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        ...
        if train_mode=='finetune':
            ...
            model_ft = models.resnet18(weights =  'ResNet18_Weights.IMAGENET1K_V1', progress = True)
            model_ft.to(device=device)


            # Modify fc layers to match num_classes
            num_ftrs = model_ft.fc.in_features
            model_ft.fc = nn.Linear(num_ftrs,num_classes, device= "mps")
        ...
        #summary(model_ft, input_size=(3, 224, 224), device=torch.device("mps"))
        print(model_ft)
        ...
        def train_model(model, criterion, optimizer, scheduler, num_epochs=30):
            ...
            # Tensorboard summary
            writer = SummaryWriter(log_dir=f'logs/{train_mode}', comment=f'{train_mode}_on_{device}')
